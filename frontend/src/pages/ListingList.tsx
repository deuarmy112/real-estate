...
export default function ListingList() {
  ...
  const [pageSize, setPageSize] = useState(24) // initial page size tuned
    ...
    const observer = new IntersectionObserver((entries) => {
      entries.forEach((entry) => {
        if (entry.isIntersecting) {
          ...
          setLoadingMore(true)
          // small debounce to show spinner and avoid rapid jumps
          setTimeout(() => {
            setPageSize((prev) => Math.min(prev + 24, listings.length))
            setLoadingMore(false)
          }, 350)
        }
      })
    }, { root: null, rootMargin: '400px', threshold: 0.05 }) // larger rootMargin for earlier prefetch
      ...
      <div style={{ display: 'flex', gap: 16, alignItems: 'flex-start' }}>
        ...
        <div style={{ flex: 1 }}>
          ...
          <div style={{ marginTop: 12, textAlign: 'center' }}>
            {pageSize >= totalItems ? <div className="muted">End of results</div> : (
              <div className="loading-more">
                {loadingMore ? <Spinner /> : <div className="muted">Loading more when you scroll…</div>}
              </div>
            )}
            <div ref={sentinelRef} />
          </div>
